...
} from './neural_handlers.js';

// Import Collective Intelligence Handlers (Brain Evolution Step 6)
import {
    handleHiveAggregate,
    handleGetRegionalWeakness,
    handleNationalHeatmap,
    handleNationalVelocity
} from './regional_analytics.js';
...
} from './infinite_lore_generator.js';
...
import { authMiddleware, createSession } from './auth_middleware.js';
import { rateLimitMiddleware } from './rate_limiter.js';
import { handleSecureLogin } from './secure_endpoint_examples.js';

// Import Load Testing Optimized Handlers
import {
    handleAutoSave,
...
} from './writing_queue_handler.js';

import { callIELTSAI, validateGradingResult } from './ielts_ai_grader.js';

// Import Onboarding & Routing (Antigravity Step 4)
import { handleOnboarding } from './onboarding_handler.js';
import { handleNextMission } from './mission_routing.js';

// Import Mini-Games
import { handleMiniGameValidate } from './minigames/sentence_architect.js';
...
export default {
    ...
    /**
     * Queue Consumer Handler - Enhanced 2026 IELTS Examiner
     * Processes grading tasks from GRADING_QUEUE with:
     * - Cullen Checksum pedagogical validation
     * - Detailed corrections with categorization
     * - Retry logic with exponential backoff
     * - Analytics tracking
     */
    async queue(batch, env) {
        const { processGradingQueue } = await import('./grading_consumer.js');
        return await processGradingQueue(batch, env);
    },

    /**
     * scheduled Event - Antigravity Engine Room (Passive Engine)
     * Applies 24-hour Momentum Decay (-5%) to all inactive users.
     */
    async scheduled(event, env, ctx) {
        ctx.waitUntil(this.handleDailyDecay(env));
    },

    async handleDailyDecay(env) {
        console.log('[CRON] Applying Passive Engine Gravity (Momentum Decay)...');

        // 1. Decay Momentum for inactive users
        const decayResult = await env.DB.prepare(`
            UPDATE users 
            SET momentum_score = ROUND(momentum_score * 0.95, 2)
            WHERE last_active < datetime('now', '-1 day')
            AND momentum_score > 0
        `).run();

        console.log(`[CRON] Gravity applied to ${decayResult.meta.changes} users.`);

        // 2. Trigger Momentum Boost Notifications (The Matrix)
        const candidates = await env.DB.prepare(`
            SELECT user_id, username, momentum_score, last_active, boss_level_cleared 
            FROM users 
            WHERE momentum_score > 0
            AND (last_notified_at IS NULL OR last_notified_at < datetime('now', '-2 hours'))
            LIMIT 100
        `).all();
        ...
        for (const user of candidates.results) {
            ...
            if (user.boss_level_cleared === 0) {
                message = `Aura: That 'Logic Bridge' is still waiting. Don't let the Boss Level win today. ⚔️`;
            } else if (hoursSinceActive >= 23 && hoursSinceActive < 24) {
                message = `Aura: 23 hours down, 1 to go. Keep the streak alive and boost your momentum! 🔥`;
            } else if (user.momentum_score < 20) {
                message = `Aura: Your momentum is low, but your potential isn't. One 60-second task to reset? ⚡`;
            } else if (user.momentum_score < 40) {
                message = `Aura: The Frontier is quiet without you, ${user.username}. Ready to regain your velocity? 🛰️`;
            }

            if (message) {
                console.log(`[PASSIVE_DISPATCH] To: ${user.user_id}, Message: ${message}`);
                await env.DB.prepare(`UPDATE users SET last_notified_at = datetime('now') WHERE user_id = ?`)
...
async function handleApiRoutes(url, request, env, corsHeaders) {
    ...
    // Route: PUT /api/writing/save (KV-optimized auto-save)
    ...
    // Route: POST /api/writing/submit (ASYNC - Queued grading with instant response)
    if (url.pathname === '/api/writing/submit' && request.method === 'POST') {
        return handleSubmitAsync(request, env, corsHeaders);
    }

    // Route: GET /api/submissions/:submissionId (Polling endpoint for grading status)
    if (url.pathname.startsWith('/api/submissions/') && request.method === 'GET') {
        const submissionId = url.pathname.split('/')[3];
        const userId = url.searchParams.get('userId'); // Optional user verification
        return handleGetSubmissionStatus(submissionId, userId, env, corsHeaders);
    }
    ...
    // ========== R2 PRESIGNED UPLOAD ROUTES (PILLAR 1) ==========

    // Route: POST /api/speaking/get-upload-url
    if (url.pathname === '/api/speaking/get-upload-url' && request.method === 'POST') {
    ...
    // ========== ANTIGRAVITY ONBOARDING & ROUTING ==========

    // Route: POST /api/onboarding
    if (url.pathname === '/api/onboarding' && request.method === 'POST') {
        return handleOnboarding(request, env, corsHeaders);
    }

    // Route: GET /api/missions/next
    if (url.pathname === '/api/missions/next' && request.method === 'GET') {
        return handleNextMission(request, env, corsHeaders);
    }

    // ========== MINI-GAMES ==========

    // Route: POST /api/minigame/validate
    if (url.pathname === '/api/minigame/validate' && request.method === 'POST') {
        return handleMiniGameValidate(request, env, corsHeaders);
    }
    ...
    // ========== PROFESSIONAL-GRADE FEEDBACK SYSTEM ROUTES ==========

    // Route: POST /api/speaking/submit-advanced (Confidence-based transcription with fluency analysis)
    if (url.pathname === '/api/speaking/submit-advanced' && request.method === 'POST') {
        try {
            ...
            if (!audioFile || !userId) {
                return new Response(JSON.stringify({ error: 'Missing audio or user_id' }), {
                    status: 400,
            ...
            const { generateSpeakingFeedbackWithConfidence } = await import('./confidence_speaking_consumer.js');

            const result = await generateSpeakingFeedbackWithConfidence(audioBuffer, userId, env);

            return new Response(JSON.stringify(result), {
                headers: { ...corsHeaders, 'Content-Type': 'application/json' }
            });
        } catch (error) {
            console.error('Advanced speaking feedback error:', error);
    ...
    // Route: GET /api/speaking/submission/:submissionId (Get full submission with confidence metadata)
    if (url.pathname.startsWith('/api/speaking/submission/') && request.method === 'GET') {
        const submissionId = url.pathname.split('/')[4];
        try {
            const submission = await env.DB.prepare(`
                SELECT * FROM speaking_submissions WHERE id = ?
            ...
            // Parse JSON fields
            submission.confidence_data = JSON.parse(submission.confidence_data || '[]');
            submission.pause_data = JSON.parse(submission.pause_data || '[]');
            submission.feedback = JSON.parse(submission.feedback || '{}');

            return new Response(JSON.stringify(submission), {
                headers: { ...corsHeaders, 'Content-Type': 'application/json' }
            });
        } catch (error) {
            console.error('Get speaking submission error:', error);
            return new Response(JSON.stringify({ error: error.message }), {
                status: 500,
                headers: { ...corsHeaders, 'Content-Type': 'application/json' }
            });
    ...
    // Route: GET /api/progress/:userId/:skillDomain (Get progress history for charts)
    if (url.pathname.startsWith('/api/progress/') && request.method === 'GET') {
        const pathParts = url.pathname.split('/');
        const userId = pathParts[3];
        const skillDomain = pathParts[4]; // 'speaking' or 'writing'

        try {
            const { results } = await env.DB.prepare(`
                SELECT raw_score, rounded_band, submission_id, submission_type, recorded_at as date
                FROM progress_history
                WHERE user_id = ? AND skill_domain = ?
                ORDER BY recorded_at ASC
            `).bind(userId, skillDomain).all();

            return new Response(JSON.stringify({ progressData: results }), {
                headers: { ...corsHeaders, 'Content-Type': 'application/json' }
            });
        } catch (error) {
    ...
    // ========== BRAIN EVOLUTION STEP 4: NEURAL CROSS-POLLINATION ==========

    // Route: POST /api/golden-thread/extract
    if (url.pathname === '/api/golden-thread/extract' && request.method === 'POST') {
        return handleGoldenThreadExtract(request, env, corsHeaders);
    }

    // Route: GET /api/golden-thread/active-buffer/:userId
    if (url.pathname.startsWith('/api/golden-thread/active-buffer/') && request.method === 'GET') {
        const userId = url.pathname.split('/')[4];
        return handleGetActiveBuffer(request, env, corsHeaders, userId);
    }

    // Route: POST /api/golden-thread/verify
    if (url.pathname === '/api/golden-thread/verify' && request.method === 'POST') {
        return handleGoldenThreadVerify(request, env, corsHeaders);
    }

    // Route: POST /api/golden-thread/flashback
    if (url.pathname === '/api/golden-thread/flashback' && request.method === 'POST') {
        return handleGoldenThreadFlashback(request, env, corsHeaders);
    }

    // Route: POST /api/dsg/record-metrics
    if (url.pathname === '/api/dsg/record-metrics' && request.method === 'POST') {
        return handleRecordMetrics(request, env, corsHeaders);
    }

    // Route: POST /api/dsg/get-intervention
    if (url.pathname === '/api/dsg/get-intervention' && request.method === 'POST') {
        return handleGetIntervention(request, env, corsHeaders);
    }

    // Route: POST /api/dsg/success-feedback
    if (url.pathname === '/api/dsg/success-feedback' && request.method === 'POST') {
        return handleSuccessFeedback(request, env, corsHeaders);
    }

    // Route: GET /api/neural-pulse/weekly-report/:userId
    if (url.pathname.startsWith('/api/neural-pulse/weekly-report/') && request.method === 'GET') {
        const userId = url.pathname.split('/')[4];
        return handleWeeklyUserReport(request, env, corsHeaders, userId);
    }

    // Route: GET /api/neural-pulse/population-insights
    if (url.pathname === '/api/neural-pulse/population-insights' && request.method === 'GET') {
        return handlePopulationInsights(request, env, corsHeaders);
    }

    // ========== CONTENT-TO-COMBAT CONVERTER ==========

    // Route: POST /api/auto-forge (Generate personalized mission from user weaknesses)
    if (url.pathname === '/api/auto-forge' && request.method === 'POST') {
        return handleAutoForge(request, env, corsHeaders);
    }

    // ========== BRAIN EVOLUTION STEP 6: COLLECTIVE INTELLIGENCE ==========

    // Regional Analytics (Hive-Mind)
    if (url.pathname === '/api/hive/aggregate' && request.method === 'POST') {
        return handleHiveAggregate(request, env, corsHeaders);
    }

    if (url.pathname.startsWith('/api/hive/regional-weakness/') && request.method === 'GET') {
        const region = decodeURIComponent(url.pathname.split('/')[4]);
        return handleGetRegionalWeakness(request, env, corsHeaders, region);
    }

    if (url.pathname === '/api/hive/national-heatmap' && request.method === 'GET') {
        return handleNationalHeatmap(request, env, corsHeaders);
    }

    if (url.pathname === '/api/hive/national-velocity' && request.method === 'GET') {
        return handleNationalVelocity(request, env, corsHeaders);
    }

    // Auto-Event Generator
    if (url.pathname === '/api/auto-forge/regional-quest' && request.method === 'POST') {
        return handleRegionalQuestGeneration(request, env, corsHeaders);
    }

    if (url.pathname === '/api/auto-forge/daily-national-challenge' && request.method === 'POST') {
        return handleDailyNationalChallenge(request, env, corsHeaders);
    }

    if (url.pathname.startsWith('/api/auto-forge/active-regional-events/') && request.method === 'GET') {
        const userId = url.pathname.split('/')[4];
        return handleGetActiveRegionalEvents(request, env, corsHeaders, userId);
    }

    if (url.pathname === '/api/auto-forge/daily-challenge' && request.method === 'GET') {
        return handleGetDailyChallenge(request, env, corsHeaders);
    }

    // Infinite Lore Generator (News Pipeline)
    if (url.pathname === '/api/lore/scrape-news' && request.method === 'POST') {
        return handleNewsScrapingJob(request, env, corsHeaders);
    }

    if (url.pathname === '/api/lore/news-to-dungeon' && request.method === 'POST') {
        return handleNewsToDungeon(request, env, corsHeaders);
    }

    if (url.pathname === '/api/lore/current-events' && request.method === 'GET') {
        return handleGetCurrentEvents(request, env, corsHeaders);
    }

    if (url.pathname === '/api/lore/auto-generate-dungeons' && request.method === 'POST') {
        return handleAutoGenerateDungeons(request, env, corsHeaders);
    }

    // Sultan's API (B2G/B2B Intelligence)
    if (url.pathname === '/api/sultan/partner/register' && request.method === 'POST') {
        return handleRegisterPartner(request, env, corsHeaders);
    }

    if (url.pathname === '/api/sultan/government/literacy-heatmap' && request.method === 'GET') {
        const apiKey = url.searchParams.get('api_key');
        return handleGovernmentLiteracyHeatmap(request, env, corsHeaders, apiKey);
    }

    if (url.pathname === '/api/sultan/university/elite-profiles' && request.method === 'GET') {
        const apiKey = url.searchParams.get('api_key');
        return handleUniversityEliteProfiles(request, env, corsHeaders, apiKey);
    }

    if (url.pathname.match(/\/api\/sultan\/university\/claim-profile\/\d+/) && request.method === 'POST') {
        const profileId = url.pathname.split('/')[5];
        const apiKey = url.searchParams.get('api_key');
        return handleClaimProfile(request, env, corsHeaders, apiKey, profileId);
    }

    if (url.pathname === '/api/sultan/weakness-stream' && request.method === 'GET') {
        const apiKey = url.searchParams.get('api_key');
        return handleWeaknessStream(request, env, corsHeaders, apiKey);
    }

    // === SECURITY ENDPOINTS ===

    // Authentication: Secure login with Telegram WebApp validation
    if (url.pathname === '/api/auth/login' && request.method === 'POST') {
        return handleSecureLogin(request, env, corsHeaders);
    }

    // GDPR: Export user data (Article 15 - Right to Access)
    if (url.pathname === '/api/gdpr/export' && request.method === 'POST') {
        try {
            const { exportUserData } = await import('./gdpr_compliance.js');
            const { autoDecrypt } = await import('./encryption_service.js');
            ...
            if (userData.personal_data?.profile && env.ENCRYPTION_KEY) {
    ...
    if (url.pathname === '/api/gdpr/delete' && request.method === 'POST') {
        try {
            const { deleteUserData } = await import('./gdpr_compliance.js');
            ...
            const report = await deleteUserData(env.DB, userId, 'user_request');
            ...
            try {
                const geminiResponse = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash-exp:generateContent?key=${env.GEMINI_API_KEY}`, {
                    ...
                    body: JSON.stringify({
                        system_instruction: {
                            parts: [{
                                text: `You are the Cullen Neural Core. Generate a gamified IELTS mission based on teaching points. The mission must directly target the user's weakness.`
                            }]
                        },
                        contents: [{
                            parts: [{
...
Return ONLY valid JSON, no markdown.`
                ...
                // Save to user_weakness_missions
                await env.DB.prepare(`
