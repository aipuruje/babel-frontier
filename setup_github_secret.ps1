Write-Host "Setting up GitHub Secret for Cloudflare Deployment" -ForegroundColor Green
...
Write-Host "Installing required module..." -ForegroundColor Yellow
# Install libsodium wrapper if not available
if (-not (Get-Module -ListAvailable -Name Sodium)) {
    # We'll use a simple Python script instead
    Write-Host "Creating Python helper script..." -ForegroundColor Yellow
...
"@
    ...
    Set-Content -Path "temp_add_secret.py" -Value $pythonScript
    
    Write-Host "Installing Python dependencies..." -ForegroundColor Yellow
    python -m pip install --quiet pynacl requests
    
    Write-Host "Running secret setup script..." -ForegroundColor Yellow
    python temp_add_secret.py
    
    if ($LASTEXITCODE -eq 0) {
        Write-Host "✓ GitHub secret configured successfully!" -ForegroundColor Green
        Remove-Item "temp_add_secret.py" -Force
    }
    else {
    ...
    Write-Host "Using PowerShell Sodium module..." -ForegroundColor Yellow
    # Alternative PowerShell approach
}

Write-Host "`n✓ Setup complete! You can now trigger the GitHub Actions workflow." -ForegroundColor Green
