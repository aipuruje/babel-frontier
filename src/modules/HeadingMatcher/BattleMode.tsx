...
const BATTLE_CHALLENGES: BattleChallenge[] = [
    { id: 1, name: 'Quick Match', description: '5 headings in 5 minutes', timeLimit: 300, questionCount: 5, difficulty: 'Easy', xpReward: 30 },
    { id: 2, name: 'Main Idea Rush', description: '8 headings in 10 minutes', timeLimit: 600, questionCount: 8, difficulty: 'Medium', xpReward: 55 },
    { id: 3, name: 'Distractor Destroyer', description: '12 headings in 15 minutes', timeLimit: 900, questionCount: 12, difficulty: 'Hard', xpReward: 80 }
];
...
export default function BattleMode() {
    ...
    useEffect(() => {
        const saved = localStorage.getItem('heading-matcher-battle-bests');
    ...
    const handleBattleComplete = useCallback(() => {
        ...
        updateProgress('heading-matcher', { accuracy: simulatedAccuracy, timeSpent: selectedChallenge.timeLimit - timeRemaining, questionsCompleted: selectedChallenge.questionCount, masteryLevel: simulatedAccuracy });
        ...
        if (simulatedAccuracy > currentBest) {
            ...
            localStorage.setItem('heading-matcher-battle-bests', JSON.stringify(newBests));
        }
        ...
        return (
            <div className="battle-active">
                ...
                <div className="battle-progress">
                    ...
                    <p>Match headings to paragraphs at high speed. Click below to simulate completion.</p>
    ...
    return (
        <motion.div className="battle-mode" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            <div className="battle-intro"><Swords size={48} className="battle-icon" /><h2>Battle Mode</h2><p>Speed-match headings to paragraphs! Avoid distractors under time pressure.</p></div>
                    ...
                    return (
                        <motion.div key={challenge.id} className={`challenge-card difficulty-${challenge.difficulty.toLowerCase()}`} whileHover={{ scale: 1.02 }} onClick={() => { triggerHaptic('selection'); setSelectedChallenge(challenge); setTimeRemaining(challenge.timeLimit); setBattleComplete(false); }}>
                            ...
                            <div className="challenge-meta">
                                ...
                                <div className="meta-item"><Target size={16} /><span>{challenge.questionCount} headings</span></div>
            ...
            {selectedChallenge && !battleActive && (
                <motion.div className="challenge-details" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                    ...
                    <div className="detail-stats">
                        ...
                        <div className="detail-stat"><strong>{selectedChallenge.questionCount}</strong> headings</div>
                    </div>
