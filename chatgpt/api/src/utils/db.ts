...
export async function getPlayerState(env: Env, userId: string): Promise<Record<string, unknown> | null> {
  ...
  return {
    ...
    streak: row.streak,
    fatigue: row.fatigue,
    mastery: JSON.parse(String(row.mastery_json)),
    errorFingerprint: JSON.parse(String(row.error_fingerprint_json)),
    updatedAt: row.updated_at
  };
}

export async function getQuestJson(env: Env, questId: string): Promise<Record<string, unknown> | null> {
  const row = await env.DB.prepare("SELECT json FROM quests WHERE id = ?1").bind(questId).first<{ json: string }>();
  return row?.json ? JSON.parse(row.json) as Record<string, unknown> : null;
}
...
export async function recordAttempt(env: Env, userId: string, questId: string, score: number, maxScore: number, errors: Array<Record<string, unknown>>, timeSpentMs: number): Promise<void> {
  await env.DB.prepare(
